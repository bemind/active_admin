      ...
      class IndexTableFor < ::ActiveAdmin::Views::TableFor
        ...
        def default_actions(options = {})
          ...
          column options[:name] do |resource|
            links = ''
            if (resource.respond_to?('ativo') && resource.ativo) or (resource.respond_to?('status') && resource.status == "ativo") or
                !resource.respond_to?('ativo') or !resource.respond_to('status')
              links = link_to I18n.t('active_admin.view'), resource_path(resource), :class => "member_link view_link"
              links += link_to I18n.t('active_admin.edit'), edit_resource_path(resource), :class => "member_link edit_link"
              links += link_to I18n.t('active_admin.delete'), resource_path(resource), :method => :delete, :confirm => I18n.t('active_admin.delete_confirmation'), :class => "member_link delete_link"
            else
              links += link_to I18n.t('active_admin.activate'), activate_resource_path(resource), :method => :put, :confirm => I18n.t('active_admin.delete_confirmation'), :class => "member_link activate_link"
            end
